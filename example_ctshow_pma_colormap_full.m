...
clt_pma = readtable(fullfile(path_data,'PMA_lut.csv'));
...
load(fullfile(path_data,'CBF.mat')); % precalculated CBF
load(fullfile(path_data,'CBV.mat')); % precalculated CBV
...
% default colormap
p1 = subplot(5,2,1);
[~,cm1,~] = ctshow_pma(CBF,mask,[0 60],'default');
title('CBV default');

p2 = subplot(5,2,2);
[~,cm2,~] = ctshow_pma(CBV,mask,[0 30],'default');
title('CBV default');

% PMA colormap ASIST
p3 = subplot(5,2,3);
[~,cm3,~] = ctshow_pma(CBF,mask,[0 100],'pma',CLT_ASIST);
title('CBF PMA colormap ASIST');

p4 = subplot(5,2,4);
[~,cm4,~] = ctshow_pma(CBV,mask,[0 40],'pma',CLT_ASIST);
title('CBV PMA colormap ASIST');

% PMA colormap Siemens_CT
p5 = subplot(5,2,5);
[~,cm5,~] = ctshow_pma(CBF,mask,[0 100],'pma',CLT_Siemens_CT);
title('CBF PMA colormap Siemens CT');

p6 = subplot(5,2,6);
[~,cm6,~] = ctshow_pma(CBV,mask,[0 40],'pma',CLT_Siemens_CT);
title('CBV PMA colormap Siemens CT');

% PMA colormap Philips_CBF & Philips_CBV
p7 = subplot(5,2,7);
[~,cm7,~] = ctshow_pma(CBF,mask,[0 100],'pma',CLT_Philips_CBF);
title('CBF PMA colormap Philips CBF');

p8 = subplot(5,2,8);
[~,cm8,~] = ctshow_pma(CBV,mask,[0 40],'pma',CLT_Philips_CBV);
title('CBV PMA colormap Philips CBV');

% PMA colormap PGE_3_Colors
p9 = subplot(5,2,9);
[~,cm9,~] = ctshow_pma(CBF,mask,[0 100],'pma',CLT_GE_3_Colors);
title('CBF PMA colormap GE 3 Colors');

p10 = subplot(5,2,10);
[~,cm10,~] = ctshow_pma(CBV,mask,[0 40],'pma',CLT_GE_3_Colors);
